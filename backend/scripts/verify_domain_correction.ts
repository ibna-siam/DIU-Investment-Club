...
process.env.CLIENT_URL = 'https://invesmentclub.top,https://www.invesmentclub.top,https://diu-investment-club-nine.vercel.app';
process.env.JWT_SECRET = 'a_very_long_secure_jwt_secret_token_at_least_32_chars_long!';
process.env.RESEND_FROM_EMAIL = 'DIU Investment Club <noreply@invesmentclub.top>';
...
console.log('  Target Domain: https://invesmentclub.top');
console.log('====================================================\n');

// 1. Primary Client URL resolution
runTest('getPrimaryClientUrl() strips commas and resolves https://invesmentclub.top', () => {
  const primary = getPrimaryClientUrl();
  assert.strictEqual(primary, 'https://invesmentclub.top');
});

// 2. Email Sender Identity
runTest('DEFAULT_FROM_EMAIL uses verified domain noreply@invesmentclub.top', () => {
...
runTest('EMAIL_BRAND portalUrl resolves to https://invesmentclub.top', () => {
  assert.strictEqual(EMAIL_BRAND.portalUrl, 'https://invesmentclub.top');
});

// 4. Password Reset URL Structure
runTest('Password reset URL correctly generates https://invesmentclub.top/reset-password', () => {
  const token = 'sample_secure_token_123';
  const resetUrl = `${getPrimaryClientUrl()}/reset-password?token=${token}`;
  assert.strictEqual(resetUrl, 'https://invesmentclub.top/reset-password?token=sample_secure_token_123');
  assert.strictEqual(resetUrl.includes(','), false);
});

// 5. Account Setup / Invitation URL Structure
runTest('Account invitation setup URL correctly generates https://invesmentclub.top/reset-password?setup=true', () => {
  const email = 'user@diu.edu.bd';
  const setupUrl = `${getPrimaryClientUrl()}/reset-password?setup=true&email=${encodeURIComponent(email)}`;
  assert.strictEqual(setupUrl, 'https://invesmentclub.top/reset-password?setup=true&email=user%40diu.edu.bd');
  assert.strictEqual(setupUrl.includes(','), false);
});

// 6. Login URL in Welcome Email
runTest('Login URL in welcome email correctly generates https://invesmentclub.top/login', () => {
  const loginUrl = `${getPrimaryClientUrl()}/login`;
  assert.strictEqual(loginUrl, 'https://invesmentclub.top/login');
});

// 7. CORS Origins Validation
runTest('CORS origins allow invesmentclub.top and reject old wrong domain investmentclub.top', () => {
  ...
  assert.strictEqual(isAllowed('https://invesmentclub.top'), true, 'Must allow https://invesmentclub.top');
  assert.strictEqual(isAllowed('https://www.invesmentclub.top'), true, 'Must allow https://www.invesmentclub.top');
