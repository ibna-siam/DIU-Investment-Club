...
async function runPhase3Verification() {
  ...
  // 1. Verify Server Health & Module Readiness
  ...
  // Dynamically import compiled modules using ts-node or direct service import
  const { emailEventBus } = require('../dist/modules/email/email.events.js').emailEventBus
    ? require('../dist/modules/email/email.events.js')
    : await import('../src/modules/email/email.events.ts');

  const { emailQueue } = require('../dist/modules/email/email.queue.js').emailQueue
    ? require('../dist/modules/email/email.queue.js')
    : await import('../src/modules/email/email.queue.ts');

  const { emailRepository } = require('../dist/modules/email/email.repository.js').emailRepository
    ? require('../dist/modules/email/email.repository.js')
    : await import('../src/modules/email/email.repository.ts');
  ...
  await sleep(1500); // Allow async queue to process
  const log1 = await emailRepository.findByIdempotencyKey(`USER_CREATED:${test1UserId}`);
  ...
  await sleep(1500);
  const log2 = await emailRepository.findByIdempotencyKey(`PAYMENT_CONFIRMED:${test2PaymentId}`);
  ...
  await sleep(1500);
  const log3 = await emailRepository.findByIdempotencyKey(`EXPENSE_SUBMITTED:${test3ExpenseId}:${RESEND_TEST_RECIPIENT}`);
  ...
  await sleep(1500);
  const log4 = await emailRepository.findByIdempotencyKey(`EXPENSE_APPROVED:${test4ExpenseId}`);
  ...
  await sleep(1500);
  const log5 = await emailRepository.findByIdempotencyKey(`EXPENSE_REJECTED:${test5ExpenseId}`);
  ...
  await sleep(1500);
  const log6 = await emailRepository.findByIdempotencyKey(`EVENT_CREATED:${test6EventId}:${RESEND_TEST_RECIPIENT}`);
  ...
  await sleep(1500);
  const log7 = await emailRepository.findByIdempotencyKey(`MEETING_SCHEDULED:${test7MeetingId}:${RESEND_TEST_RECIPIENT}`);
  ...
  await sleep(1500);
  const log8 = await emailRepository.findByIdempotencyKey(`TASK_ASSIGNED:${test8TaskId}:${test8AssigneeId}`);
  ...
  // Custom execution verifying retries
  const testJob = {
    id: 'job-with-retries',
    idempotencyKey: retryKey,
    emailType: 'RETRY_TEST',
    category: 'GENERAL',
    recipient: RESEND_TEST_RECIPIENT,
    subject: 'Retry Test Email',
    html: '<p>Testing retry handling</p>',
    maxRetries: 3,
  };
  ...
  assert(logsData.total >= 8, `Logs total count is populated (${logsData.total} logs recorded)`);
  assert(logsData.totalPages >= 1, `Pagination metadata included (totalPages: ${logsData.totalPages})`);

  // Verify non-leakage of passwords/secrets
